...
<p align="center">
  <em>A multi-agent AI council for decisions and research: Advisors debate to a recommendation,
  Researchers retrieve cited evidence — solo or chained — installable across every AI agent
  (Kiro, Cursor, Claude, Windsurf, and more).</em>
</p>
...
- **Chained** — researchers first, then advisors reason over the findings, for _"what should we do,
  given the current landscape?"_ The recommendation cites the evidence it rests on, and dissent is
...
# Custom Minds — scaffold your own advisors and researchers
council advisor create <name>       # Scaffold a custom advisor from the template
council advisor validate <name>     # Check a custom advisor's structure
council advisor list                # List custom advisors
council researcher create <name>    # Scaffold a custom researcher from the template
council researcher validate <name>  # Check a custom researcher's structure
...
### Research — Ask What Is True

```
research council: Is it true that Postgres now outperforms DynamoDB for high-write event stores?
```

Returns a cited answer with per-claim confidence and falsifiers, not opinions.

### Chained — Decide, Given the Evidence

```
council this: Which event store should we adopt, given the current landscape?
```

Researchers retrieve the facts first, then advisors reason over them — the recommendation cites
the evidence it rests on, with dissent preserved at both layers.
...
A **research profile** is a named panel of researchers tuned for a retrieval goal — the researcher
equivalent of an advisor profile. Every panel satisfies the selector constraint (at least one
Synthesis and one Adversarial researcher). Canonical rosters and the "why" live in
[docs/researchers.md](docs/researchers.md) and
[`settings/research-council.config.json`](settings/research-council.config.json).

| Profile                     | Researchers                                                                                            | Best For                             |
| --------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------ |
...
> **Adversarial evidence auditing** — researchers audit each other's _evidence_ (provenance,
...
**Chaining:** a compound question — _what should I do, given the current
landscape_ — runs the researchers first, then feeds their cited Research Verdict into an advisor
...
|  2   | **Independent Analysis** — 4-6 advisors spawn in parallel, 300 words each, evidence-labeled. _Interactive: checkpoint here_    |
|  3   | **Fact-Check** — claims scanned for verification; disputed/unverifiable flagged for Round 2                                    |
|  4   | **Cross-Examination** — structured Disagree/Strengthen/Update with sparse engagement targeting. _Interactive: checkpoint here_ |
...
|  6b  | **Vote Tally** — confidence-weighted scoring with domain-weight seat (1.5x), 2/3 threshold                                     |
...
### What is a multi-agent AI council?

Instead of asking one AI and getting one confident answer, Council of Minds runs a small panel of
distinct AI perspectives (Minds) over your question, has them cross-examine each other, and returns
where they agree, where they clash, and the strongest dissent. The disagreement is the product.

### What is the difference between an Advisor and a Researcher?

**Advisors** reason from their perspective to a **recommendation** ("what should I do?").
**Researchers** retrieve external evidence and return a **cited answer** with per-claim confidence
("what is true?"). You can run either alone, or **chain** them — research first, then advisors
reason over the findings.

### Which AI assistants does it support?

It installs into Kiro, Cursor, Claude, Windsurf, Cline, Aider, RooCode, OpenCode, GitHub Copilot,
and Codex. Council of Minds is not itself a coding tool — it is a decision-and-research layer that
runs inside whichever AI agent you already use.

### Is Council of Minds only for coding decisions?

No. It is a general decision-and-research tool. Sessions range from architecture and hiring to
acquisition offers, market landscape scans, claim verification, and strategy — anything with real
tradeoffs or citable facts.

### How is this different from AutoGen, CrewAI, or a single LLM?

Frameworks like AutoGen and CrewAI are orchestration libraries you build agents with.
Council of Minds is a ready-made council of 60 advisors and 60 researchers with a structured
debate process — anti-conformity enforcement, sparse cross-examination, fact-checking, preserved
dissent, and per-claim confidence. See [docs/comparison.md](docs/comparison.md).

### Do I need to write code or run a server?

No. Council of Minds is plain markdown — no runtime, no dependencies. The `council` CLI just copies
files into your AI agents in their native format.
